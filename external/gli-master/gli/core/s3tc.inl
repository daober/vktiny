...
namespace gli
{
	namespace detail
	{
		...
		inline glm::vec4 decompress_dxt3(const dxt3_block &Block, const extent2d &BlockTexelCoord)
		{
			...
			gli::uint8_t ColorIndex = (Block.Row[BlockTexelCoord.y] >> (BlockTexelCoord.x * 2)) & 0x3;
		...
		inline texel_block4x4 decompress_dxt3_block(const dxt3_block &Block)
		{
			...
			for(gli::uint8_t Row = 0; Row < 4; ++Row)
			{
				...
				{
					gli::uint8 ColorIndex = (Block.Row[Row] >> (Col * 2)) & 0x3;
		...
		inline glm::vec4 decompress_dxt5(const dxt5_block &Block, const extent2d &BlockTexelCoord)
		{
			...
			Bitmap |= gli::uint64_t(Block.AlphaBitmap[3] | (Block.AlphaBitmap[4] << 8) | (Block.AlphaBitmap[5] << 16)) << 24;

			gli::uint8_t AlphaIndex = (Bitmap >> ((BlockTexelCoord.y * 4 + BlockTexelCoord.x) * 3)) & 0x7;
		...
		inline texel_block4x4 decompress_dxt5_block(const dxt5_block &Block)
		{
			...
			for(gli::uint8_t Row = 0; Row < 4; ++Row)
			{
				for(gli::uint8_t Col = 0; Col < 4; ++Col)
				{
					gli::uint8_t ColorIndex = (Block.Row[Row] >> (Col * 2)) & 0x3;
					gli::uint8_t AlphaIndex = (Bitmap >> ((Row * 4 + Col) * 3)) & 0x7;
