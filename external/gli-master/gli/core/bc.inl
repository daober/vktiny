...
namespace gli
{
	namespace detail
	{
		...
		inline void single_channel_bitmap_data_unorm(gli::uint8_t Channel0, gli::uint8_t Channel1, const gli::uint8_t *ChannelBitmap, float *LookupTable, gli::uint64_t &ContiguousBitmap)
		{
			...
			ContiguousBitmap |= gli::uint64_t(ChannelBitmap[3] | (ChannelBitmap[4] << 8) | (ChannelBitmap[5] << 16)) << 24;
		}

		inline void single_channel_bitmap_data_snorm(gli::uint8_t Channel0, gli::uint8_t Channel1, const gli::uint8_t *ChannelBitmap, float *LookupTable, gli::uint64_t &ContiguousBitmap)
		{
		...
		inline void single_channel_bitmap_data_snorm(gli::uint8_t Channel0, gli::uint32_t Channel1, bool Interpolate6, const gli::uint8_t *ChannelBitmap, float *LookupTable, gli::uint64_t &ContiguousBitmap)
		{
			LookupTable[0] = (Channel0 / 255.0f) * 2.0f - 1.0f;
			LookupTable[1] = (Channel1 / 255.0f) * 2.0f - 1.0f;
		...
		inline glm::vec4 decompress_bc4unorm(const bc4_block &Block, const extent2d &BlockTexelCoord)
		{
			...
			gli::uint8_t RedIndex = (Bitmap >> ((BlockTexelCoord.y * 4 + BlockTexelCoord.x) * 3)) & 0x7;
		...
		inline texel_block4x4 decompress_bc4unorm_block(const bc4_block &Block)
		{
			...
			for(gli::uint8_t Row = 0; Row < 4; ++Row)
			{
				for(gli::uint8_t Col = 0; Col < 4; ++Col)
				{
					gli::uint8_t RedIndex = (Bitmap >> ((Row * 4 + Col) * 3)) & 0x7;
		...
		{
			float RedLUT[8];
			gli::uint64_t RedBitmap;

			single_channel_bitmap_data_unorm(Block.Red0, Block.Red1, Block.RedBitmap, RedLUT, RedBitmap);
			gli::uint8_t RedIndex = (RedBitmap >> ((BlockTexelCoord.y * 4 + BlockTexelCoord.x) * 3)) & 0x7;
			
			float GreenLUT[8];
			gli::uint64_t GreenBitmap;

			single_channel_bitmap_data_unorm(Block.Green0, Block.Green1, Block.GreenBitmap, GreenLUT, GreenBitmap);
			gli::uint8_t GreenIndex = (GreenBitmap >> ((BlockTexelCoord.y * 4 + BlockTexelCoord.x) * 3)) & 0x7;
		...
		inline texel_block4x4 decompress_bc5unorm_block(const bc5_block &Block)
		{
			...
			for(gli::uint8_t Row = 0; Row < 4; ++Row)
			{
				for(gli::uint8_t Col = 0; Col < 4; ++Col)
				{
					gli::uint8_t RedIndex = (RedBitmap >> ((Row * 4 + Col) * 3)) & 0x7;
					gli::uint8_t GreenIndex = (GreenBitmap >> ((Row * 4 + Col) * 3)) & 0x7;
