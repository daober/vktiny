...
namespace detail
{
	...
	inline void flip_block_s3tc(gli::uint8_t* BlockDst, gli::uint8_t* BlockSrc, format Format, bool HeightTwo)
	{
	...
	inline void flip_s3tc(image ImageDst, image ImageSrc, format Format)
	{
		...
		if(ImageSrc.extent().y == 2)
		{
			for(std::size_t i_block = 0; i_block < XBlocks; ++i_block)
				flip_block_s3tc(ImageDst.data<gli::uint8_t>() + i_block * block_size(Format), ImageSrc.data<gli::uint8_t>() + i_block * block_size(Format), Format, true);
		...
		for(std::size_t i_row = 0; i_row <= MaxYBlock; ++i_row)
			for(std::size_t i_block = 0; i_block < XBlocks; ++i_block)
				flip_block_s3tc(ImageDst.data<gli::uint8_t>() + (MaxYBlock - i_row) * block_size(Format) * XBlocks + i_block * block_size(Format), ImageSrc.data<gli::uint8_t>() + i_row * block_size(Format) * XBlocks + i_block * block_size(Format), Format, false);
	}
